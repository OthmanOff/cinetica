...
export class MovieRepositoryTMDB implements MovieRepository {
  ...
  async getImagesMovie(id: number): Promise<Image[]> {
    try {
      const req = await fetch(`/api/movies/${id}/images`);
      const data = await req.json();
      return data.data as Promise<Image[]>;
    } catch {
      throw new Error("Error while fetching : /api/movies/${id}/images");
    }
  }
}
